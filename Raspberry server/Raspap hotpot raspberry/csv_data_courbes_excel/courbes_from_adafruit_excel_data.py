...
PATH = '/home/thenes/database/Classeur1.xlsx'
...
for i in range(len(sheet['A'])-1):
    x.append(sheet.cell(row=i+2,column=4).value[11::])  #HEURE   -> date + heure : x.append(sheet.cell(row=i+2,column=7).value)
    y1.append(sheet.cell(row=i+2,column=2).value) #capteur1
...
plt.plot(x, y1, "r--", label="Temperature") # Récupérer nom dans l'excel : label=sheet['D1'].value)
