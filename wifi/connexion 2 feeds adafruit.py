...
while True:
    ...
    print("Sending Lux")
    li_data = li.light()
    lumiere = str(li_data[0])
    print("la lumiere est de : ", lumiere)
    client1.publish(topic="PFE_Pycom/feeds/lumiere", msg=lumiere)

    client1.check_msg()

    pycom.rgbled(0xFF0000)
    time.sleep(5) # boucle pour envoyer les données toutes les X secondes
    pycom.rgbled(0x000000) # permet de faire clignoter la led en rouge à chaque envoi de donnée
